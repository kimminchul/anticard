...
const EXPANDABLE_POSTS: ExpandablePost[] = [
  {
    id: 1,
    title: "안티 카드 v0.13 — 카드 안에 카드를 쌓지 않는다",
    category: "Lab",
    summary:
      "박스 안에 박스를 쌓는 shadcn 톤을 거부하고 1px 헤어라인 + 행만으로 위계를 만드는 anti-card 라이브러리의 P7 마무리.",
    publishedAt: "2026-05-09",
  },
  {
    id: 2,
    title: "Heritage — 운영자의 깊이를 1인 랩으로",
    category: "Heritage",
    summary:
      "2016~2022 Preive 시기의 30곳+/150건+ 프로젝트를 '레거시'가 아니라 '권위 근거'로 다시 정리한 페이지의 회고.",
    publishedAt: "2026-05-05",
  },
  {
    id: 3,
    title: "Pretendard 폰트와 안티 카드 톤",
    category: "Note",
    summary:
      "Pretendard variable + ss01·ss02 OpenType feature가 어떻게 안티 카드 1px 헤어라인 톤과 자연스럽게 맞물리는지.",
    publishedAt: "2026-04-28",
  },
];
function ExpandableTableDemo() {
  return (
    <ExpandableTable<ExpandablePost>
      data={EXPANDABLE_POSTS}
      single
      toggleOnRowClick
      expandKey={(p) => String(p.id)}
      columns={[
        { key: "title", header: "제목", sortable: true },
        { key: "category", header: "카테고리", sortable: true, width: "w-[100px]" },
        { key: "publishedAt", header: "발행일", sortable: true, align: "right", width: "w-[110px]" },
      ]}
      renderExpanded={(p) => (
        <div className="space-y-2 text-[13px] leading-relaxed text-zinc-600 dark:text-zinc-300">
          <p>{p.summary}</p>
          <p className="text-[11.5px] uppercase tracking-[0.08em] text-zinc-400">
...
const EXPANDABLE_TABLE_DEF: ComponentDef = {
  id: "expandable-table",
  ko: "펼침 가능 테이블",
  en: "ExpandableTable",
  desc: "DataTable + 행 펼침 (chevron + colSpan 상세 panel). 카드 그리드의 대안 — 표는 좁게, 깊은 정보는 같은 줄 컨텍스트로.",
  intro:
    "표의 한 행을 클릭하면 바로 그 아래에 펼쳐지는 상세 영역으로 추가 정보를 보여 주는 표입니다. 모든 정보를 좌우로 넓히지 않고도 행마다 본문·메타·이력 같은 깊은 데이터를 같은 줄 컨텍스트에서 볼 수 있어, 카드 그리드를 쓰지 않고도 풍부한 정보 페이지를 만들 수 있습니다.",
  useCases: [
    "Admin 글 리스트 — 행 클릭으로 본문 미리보기 / 메타 / 액션",
    "Heritage 사례 리스트 — 행 펼침으로 프로젝트 회고·결과",
    "주문·로그 리스트 — 한 행 아래에 상세 trace 풀어 보기",
    "FAQ 표 — 질문 행 + 답변 펼침 (Tabs/FAQ 대안)",
  ],
  examples: [
    {
      index: "01",
      badge: "interactive",
      title: "행 클릭으로 펼침 (accordion)",
      description:
        "single=true → 한 번에 하나만 펼침 (다른 행 펼치면 기존 닫힘). toggleOnRowClick=true → 행 어디든 클릭으로 toggle.",
      preview: <ExpandableTableDemo />,
      prompt: `ExpandableTable — 카드 그리드의 대안.

- 첫 컬럼: chevron 버튼 (ChevronRight, 펼침 시 rotate-90)
- 펼침 시 다음 줄 colSpan=N으로 renderExpanded 결과
- 펼친 행 자체: bg-zinc-50/60 (light) / bg-white/[0.02] (dark) — 헤어라인 톤 유지
- single=true면 accordion (한 개만), false면 다중 펼침
- toggleOnRowClick=true면 행 어디든 클릭으로 toggle (chevron 외에도)
- 행 onClick과 chevron onClick stopPropagation으로 분리 (필요 시)`,
      react: `<ExpandableTable<Post>
  data={posts}
  expandKey={(p) => String(p.id)}
  single
  toggleOnRowClick
...
/>`,
  ...
  props: [
    { name: "data / columns / rowKey / density / caption", type: "...", desc: "DataTable과 동일" },
    { name: "expandKey", type: "(row, i) => string", desc: "펼침용 unique key (default: rowKey > index)" },
    { name: "defaultExpandedKeys", type: "string[]", desc: "uncontrolled 초기 펼침" },
    { name: "expandedKeys", type: "string[]", desc: "controlled 펼친 키" },
    { name: "onExpandedChange", type: "(keys: string[]) => void", desc: "펼침 변경 콜백" },
